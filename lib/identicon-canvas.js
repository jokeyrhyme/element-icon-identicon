...
(function (root, factory) {
  ...
  function fixCode(code) {
    if (typeof code === 'string') {
      ...
      code = code.replace(/\W|_/g, '');
      if (code.match(/^\d+\.?\d*e(\+|-)?\d+$/)) {
        code = parseFloat(code, 10);
      } else if (!code.match(/[^0-9A-F]/)) {
  ...
  function render_identicon (node, code, size) {
    ...
    // scaling as per: http://www.html5rocks.com/en/tutorials/canvas/hidpi/

    var ctx, devicePixelRatio, backingStoreRatio, pixelRatio, backingSize;
    ...
    backingStoreRatio = ctx.webkitBackingStoreRatio ||
      ctx.mozBackingStoreRatio ||
      ctx.msBackingStoreRatio ||
      ctx.oBackingStoreRatio ||
      ctx.backingStoreRatio;

    pixelRatio = backingStoreRatio || devicePixelRatio;
    node.style.height = size + 'px';
    node.style.width = size + 'px';

    backingSize = size * pixelRatio;
    node.setAttribute('height', backingSize);
    node.setAttribute('width', backingSize);

    // END: scaling

    var patchSize = backingSize / 3;
  ...
  return {
    fixCode: fixCode,
    render: render_identicon,
    renderPatch: render_identicon_patch,
    findAndRenderCanvases: render_identicon_canvases
  };
